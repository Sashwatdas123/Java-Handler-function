...
```bash
java-gtse$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
java-gtse$ sam local start-api
java-gtse$ curl http://localhost:3000/
```
...
```bash
java-gtse$ sam logs -n HelloWorldFunction --stack-name java-gtse --tail
```
...
```bash
java-gtse$ cd HelloWorldFunction
...
```bash
sam delete --stack-name java-gtse
```
